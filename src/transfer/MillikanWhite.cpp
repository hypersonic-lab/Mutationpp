    ...
    const XmlElement& node, const class Thermodynamics& thermo)
{
    ...
    for (int i = 0; i < thermo.nHeavy(); ++i) {
        ...
        if ((partner_iter = node.findTagWithAttribute(
            ...
            partner_iter->getAttribute("ml", ml, 0.0);
            partner_iter->getAttribute("nl", nl, 0.0);
            partner_iter->getAttribute("mh", mh, 0.0);
            partner_iter->getAttribute("nh", nh, 0.0);
            
            // Add Millikan-White data for collision pair
            m_partners.push_back(MillikanWhitePartner(a, b, ml, nl, mh, nh, mu));
        } else {
            if (m_thetav < 0.0) {
            ...
            m_partners.push_back(MillikanWhitePartner(mu, m_thetav,vibrator.molecularWeight(),0));
        }
    }
}
