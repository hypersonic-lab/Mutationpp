...
*/
class STSDB_AR : public ThermoDB
{
public:

    STSDB_AR(int arg) : ThermoDB(298.15, 101325.0){}
...
//     }
    ...
    double g0_Ar = 0.0; // Argon
    double g1_Ar = 3.0; // Argon 
    double theta_1_Ar = 134003.0; // Argon – Approximately --> NIST
    double g0_O = 5.0;
    double g1_O = 4.0;
    double theta_1_O = 270;
     ...
     if (cpel != NULL) {
         for (int i = 0; i < m_ns; i++){
            ...
            if (i == 1) {
                // cpel[i] = 0.0; // Ground state
                cpel[i] = pow((theta_1_Ar/Tel),2.0) * (g1_Ar * exp(-theta_1_Ar / Tel)); // Ground state
                cp[i] += cpel[i];
                 continue; } // Ground state
        ...
        if (cp != NULL){
         ...
         for (int i = 0; i < m_ns; i++){
            ...
            if (i == 1) {
                // cp[i] = 0.0; // Ground state
                cp[i] += pow((theta_1_Ar/Tel),2.0) * (g1_Ar * exp(-theta_1_Ar / Tel)); // Ground state
                 continue; } // Ground state
            //  cp[i] += 0.0;
...
//        energy[9] = 13961.1710261;
        ...
        if (hv != NULL) {
            ...
            for (int i = 0; i < m_ns; i++){
                ...
                hv[i] = energy[i-2] * cm2J / (KB * Th); //* exp(-1*energy[i-1] * 1.42879 / Th); // See KMH notes
                m_hv[i] = energy[i-2] * cm2J / (KB * Th);
                // hv[i] = energy[i-1] * 1.42879 / (Th) / (exp(-1.0 * energy[i-1]* 1.42879  / Th) - 1.0); //* exp(-1*energy[i-1] * 1.42879 / Th); // See KMH notes
                h[i] += energy[i-2] * cm2J / (KB * Th);
            }}
...
//                    h[i] = 0.0; // Ground state
                ...
                m_hv[i] += energy[i-2] * cm2J / (KB * Th);   // * exp(-1*energy[i-1] * 1.42879 / Th); // See KMH notes
                ...
                h[i] += energy[i-2] * cm2J / (KB * Th); //energy[i-1] * 1.42879 / Th * exp(-1*energy[i-1] * 1.42879 / Th); // See KMH notes
            }}
        }
        ...
        if (hel != NULL) {
            ...
            for (int i = 0; i < m_ns; i++){
                if (i == 0) {
                    ...
                    continue; }
                if (i == 1) {
                    hel[i] = ((theta_1_Ar) * g1_Ar * exp(-theta_1_Ar / Tel)) / Th;
                    m_hel[i] =  ((theta_1_Ar) * g1_Ar * exp(-theta_1_Ar / Tel)) / Th;
                    h[i] +=  ((theta_1_Ar) * g1_Ar * exp(-theta_1_Ar / Tel)) / Th;
                    continue; }
...
//                    h[i] = 0.0; // Ground state
        ...
        // double g0_O = 0.0; // Argon
        // double g1_O = 3.0; // Argon 
        // double theta_1_O = 134003.0; // Argon – Approximately --> NIST
        ...
        if (st != NULL) {
            ...
            for (int i = 0; i < m_ns; i++){
                if (i == 0) {
                    ...
                    continue; }
                if (i == 1) {
                    st[i] += 2.5 * log(Th) - log(P) + log(pow((2*PI*0.039948 / NA / pow(HP,2.0)),1.5) * pow(KB,2.5)) + 2.5; // EQ 3.90 of Boyd. // Ground state
                    m_st[i] += 2.5 * log(Th) - log(P) + log(pow((2*PI*0.039948 / NA / pow(HP,2.0)),1.5) * pow(KB,2.5)) + 2.5;
                    s[i] +=  2.5 * log(Th) - log(P) + log(pow((2*PI*0.039948 / NA / pow(HP,2.0)),1.5) * pow(KB,2.5)) + 2.5;
                    continue; }
            ...
            for (int i = 0; i < m_ns; i++){
                if (i == 0) {
                    ...
                    continue; }
                if (i == 1) {
                    s[i] += 2.5 * log(Th) - log(P) + log(pow((2*PI*0.039948 / NA / pow(HP,2.0)),1.5) * pow(KB,2.5)) + 2.5; // EQ 3.90 of Boyd. // Ground state
                    m_st[i] += 2.5 * log(Th) - log(P) + log(pow((2*PI*0.039948 / NA / pow(HP,2.0)),1.5) * pow(KB,2.5)) + 2.5;
                    // s[i] += st[i];
                    continue; }
                s[i] += 2.5 * log(Th) - log(P) + log(pow((2*PI*0.0319988 / NA / pow(HP,2.0)),1.5) * pow(KB,2.5)) + 2.5;
        ...
        if (sel != NULL) {
            ...
            for (int i = 0; i < m_ns; i++){
                if (i == 0) {
                 ...
                 continue; } // Ground state
                if (i == 1) {
                    sel[i] += theta_1_Ar / Tel + log(exp(-theta_1_Ar / Tel) * g1_Ar) / Tel; // Ground state
                    m_sel[i] += theta_1_Ar / Tel + log(exp(-theta_1_Ar / Tel) * g1_Ar) / Tel; // Ground state
                    s[i] += theta_1_Ar / Tel + log(exp(-theta_1_Ar / Tel) * g1_Ar) / Tel; // Ground state
                 continue; } // Ground state
    ...
    // }
        ...
        if (gt != NULL) {
            for (int i = 0; i < m_ns; i++){
                ...
                if (i == 1) {
                    gt[i] -= 2.5 * log(Th) - log(P) + log(pow((2*PI*0.039948 / NA / pow(HP,2.0)),1.5) * pow(KB,2.5)) + 2.5;
                    continue;
                }
            ...
            for (int i = 0; i < m_ns; i++){
                ...
                if (i == 1){
                    g[i] -= 2.5 * log(Th) - log(P) + log(pow((2*PI*0.039948 / NA / pow(HP,2.0)),1.5) * pow(KB,2.5)) + 2.5;
                    continue;
                }
        ...
        if (gel != NULL) {
            for (int i = 0; i < m_ns; i++){
                if (i == 0) {
                 ...
                 continue; } // Ground state
                if (i == 1) {
                    gel[i] -= (log(g0_Ar) + log(1.0+g1_Ar/g0_Ar*exp(-theta_1_Ar/Tel)) + (g1_Ar/g0_Ar*theta_1_Ar/Tel*exp(-theta_1_Ar/Tel))/(1+(g1_Ar/g0_Ar)*exp(-theta_1_Ar/Tel))); // Ground state
                    g[i] -= (log(g0_Ar) + log(1.0+g1_Ar/g0_Ar*exp(-theta_1_Ar/Tel)) + (g1_Ar/g0_Ar*theta_1_Ar/Tel*exp(-theta_1_Ar/Tel))/(1+(g1_Ar/g0_Ar)*exp(-theta_1_Ar/Tel)));
                 continue; } // Ground state
...
Utilities::Config::ObjectProvider<STSDB_AR, ThermoDB> stsDB("Ar_STS");
